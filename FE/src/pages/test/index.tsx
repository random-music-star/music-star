...
const TestPage = () => {
  ...
  // User4 캐릭터 위치 이동하기
  const moveUser4Character = () => {
    setCharacters(prevCharacters => {
      return prevCharacters.map(character => {
        if (character.name === 'user4') {
          const nextPosition =
            (character.position + 1) % footholderRatios.length;
          return {
            ...character,
  ...
  useEffect(() => {
    ...
    // Canvas에 그리기
    function drawCanvas() {
      if (!canvas || !ctx || !backgroundImage || !footholderImage) return;
      ...
      // 배경 이미지 그리기 (왼쪽 3/4 영역만)
      ctx.drawImage(backgroundImage, 0, 0, leftSectionWidth, canvas.height);

      // 오른쪽 1/4 영역에 검정색 배경
      ctx.fillStyle = 'black';
      ctx.fillRect(leftSectionWidth, 0, canvas.width * 0.25, canvas.height);
  ...
  }, [isLoading, footholderImage, backgroundImage, characters]);

  return (
    <div className='relative max-h-screen min-h-screen max-w-screen min-w-screen overflow-hidden'>
      <canvas ref={canvasRef} className='absolute top-0 left-0 h-full w-full' />

      {/* 오른쪽 검은색 영역에 버튼 추가 */}
      <div className='absolute top-0 right-0 flex h-full w-1/4 flex-col items-center justify-center'>
        <button
          className='rounded-lg bg-blue-500 px-6 py-3 text-white hover:bg-blue-600 active:bg-blue-700'
          onClick={moveUser4Character}
        >
          Move User4 Character
        </button>
      </div>
    </div>
  );
